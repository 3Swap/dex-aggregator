...
export default function Navbar3Swap({
  HandleLaunchDapp,
}: {
  HandleLaunchDapp: () => void;
}) {
  const [bgOpen, setBgOpen] = useState(false);
  const [openDropdown, setDropdown] = useState(false);

  const visibleLink = true;
    ...
    <div className="w-full flex justify-between items-center pt-[1rem] px-5 z-10">
      ...
      <div className={`items-center gap-3 ${visibleLink ? 'flex' : 'hidden'}`}>
        ...
        <button
          ...
          className="px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg shadow-md inline-flex items-center"
        >
      ...
      <div className={`${visibleLink ? 'hidden' : 'flex relative'}`}>
        <div
          className={`${bgOpen ? 'bg-[#fff]/[.11]' : ''} p-3 rounded-full hover:cursor-pointer`}
        ...
        {openDropdown && (
          <div className="absolute w-36 bg-gradient-to-r from-indigo-600 to-purple-600 shadow-md h-fit py-3 px-2 flex flex-col items-center justify-start gap-y-5 right-0 top-16 rounded-xl">
            {links.map((element, index) => {
              if (element.type === 'function') {
                const { icon, title } = element;
                return (
                  <button
                    className="flex gap-x-3 items-center justify-first py-2 px-3 rounded-md text-white w-full hover:bg-[#fff]/[.11]"
                    key={index}
                  >
                    {icon}
                    {title}
